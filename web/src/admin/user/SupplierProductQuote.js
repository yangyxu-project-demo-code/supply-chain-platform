			...
			<li className="product">
				...
				<div className="details" style={{display:(this.state.active?'block':'none')}}>
					...
					{this.props.price || <div className="group">
						...
						<div>采购单价： ￥{(item.purchase_price||0).toFixed(2)}</div>
...
module.exports = React.createClass({
	getInitialState: function () {
		return {
			model: 'ChunRuiOASupplierProductQuote',
			selected: [],
			selected_sources: [],
			all: [],
			all_sources: []
	...
	__loadData: function (){
		...
		zn.http.post('/oa/quote/supplier_products', {
			supplier: this.props.supplier
		}).then(function (data){
			zn.preloader.close();
			if(data.status==200){
				data.result.all_sources = data.result.all.slice(0);
				data.result.selected_sources = data.result.selected.slice(0);
	...
	__addProductQuote: function (product){
		zn.dialog({
			...
			content: <zn.react.Form
				...
				hiddens={{ product_model: product.model, supplier_id: this.props.supplier }}
				merge="values"
				onSubmitSuccess={()=>this.__loadData()}
				items={[
					{ title: '采购单价', name: 'price', value: product.purchase_price, suffix: '￥', type: 'Input', attrs: { type: 'number' } },
	...
	__updatePrice: function (data){
		zn.dialog({
			...
			content: <zn.react.Form
				...
				onSubmitSuccess={()=>this.__loadData()}
				items={[
					{ title: '采购单价', name: 'price', value: data.quote_price, suffix: '￥', type: 'Input', attrs: { type: 'number' } },
	...
	searchAll: function (name){
		console.log('all, ', this.state.all_sources);
	...
	searchSelected: function (name){
        this.state.selected = this.state.selected_sources.slice(0).filter(function (item, index){
            if((item.model && item.model.toLocaleLowerCase().indexOf((name||'').toLocaleLowerCase())!=-1) || (item.zn_title && item.zn_title.toLocaleLowerCase().indexOf((name||'').toLocaleLowerCase())!=-1)){
                return true;
            }
        });
        this.forceUpdate();
	...
	__initQuote: function (){
		zn.dialog({
			...
			content: <div>
					...
					<zn.react.Form
						action='/oa/quote/init_supplier_quote'
						exts={{ supplier: this.props.supplier }}
						onSubmitSuccess={()=>this.__loadData()}
						items={[
							{ title: '基准价', name: 'price_field', type: 'Select', data: [
								{ text: '平均价', value: 'average_price' },
								{ text: '采购单价', value: 'purchase_price' },
								...
								return <Product data={item}
										...
										price={
											<div className="group">
												<div>可设置单价区间： ￥{(item.min_price||0).toFixed(2)} - ￥{(item.max_price||0).toFixed(2)}</div>
												<div>平均单价： ￥{(item.average_price||0).toFixed(2)}</div>
												<div style={{color: '#7b0e06', fontWeight: 'bold'}}>
													采购单价： ￥{(item.quote_price || item.purchase_price||0).toFixed(2)} / <span style={{textDecoration: 'line-through', color: '#CCC'}}>￥{(item.purchase_price||0).toFixed(2)}</span>
												</div>
												<div style={{color: '#7b0e06', fontWeight: 'bold'}}>
