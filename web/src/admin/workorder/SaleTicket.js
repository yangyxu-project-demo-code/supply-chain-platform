...
module.exports = React.createClass({
	...
	getInitialState: function () {
		return {
			...
			toolbarItems: [
				{ text: '发货订单查询', name: 'users', icon: 'fa-list-ul', status: 'warning' },
				{ text: '销售制单', name: 'add', icon: 'fa-plus' }
			]
		}
	},
	...
	__createInstance: function (index){
		zn.react.session.relativeJump('/workorder.create.sale.ticket');
		return;
		zn.dialog({
			title: '销售制单',
	...
	__onToolbarClick: function (item){
		switch (item.name) {
			case 'add':
				...
				break;
			case 'users':
				zn.react.session.relativeJump('/workorder.sale.ticket.detail.search');
				break;
		}
	},
	__onTableColumnRender: function (rowIndex, columnIndex, data, item, value){
		switch (columnIndex) {
			case 0:
				return <div>
					<a href={'#'+zn.react.session.fixPath('/workorder.sale.ticket.detail.list')+'?wf_id=' + data.id}><i data-tooltip="查看商品详情" className="fa fa-list-ul btn" /></a>
	...
	render:function(){
		return (
			<zn.react.Page title='销售制单' className="zn-plugin-workflow-wf-my"
